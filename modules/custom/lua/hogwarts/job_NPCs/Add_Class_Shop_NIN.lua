...
-----------------------------------
local m = Module:new('Class_Shop_NIN')
...
m:addOverride('xi.zones.Bastok_Markets.Zone.onInitialize', function(zone)
    ...
    local Oroku = zone:insertDynamicEntity({
        ...
        onTrigger = function(player, npc)
            -- NOTE: We have to use getPacketName, because the regular name is modified and being used
            --     : for internal lookups
            if player:getMainJob() == xi.job.NIN then
                ...
                if player:getMainJob() ~= xi.job.NIN and player:getMainLvl() >=30 then
